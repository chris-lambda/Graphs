...
def advt(player, direc = ''):
    ...
    for e in exits:
        if roomID not in graph:
            graph[roomID] = {e: '?'}
        elif e not in graph[roomID]:
            graph[roomID][e] = '?'

    # if coming from another room set the directino it came from to that old room
    if direc is not '':
        oppo = op_dir(direc)
        oldRoom = player.currentRoom.getRoomInDirection(oppo)
        graph[roomID][oppo] = oldRoom.id
    
    # Decide waht direction to go
    new_dir = '?'

    # check if any directions are still '?' if not the first one that isnt is the new direction
    for e in exits:
        if graph[roomID][e] == '?':
            new_dir = e
            player.travel(e)
            ...
            graph[roomID][e] = new_room
            
            advt(player, e)
            break

    # if there is no '?' find the closest '?' thorugh BFS
    walk_path = []
    if new_dir is '?':
        q = [[roomID]]
        explored = set()
        ...
        while len(q) != 0:
            ...
            if  current_room not in explored:
                ...
                for k in graph[current_room].values():
                    new_path = list(path)
                    new_path.append(k)
                    q.append(new_path)
